...
export function Navbar({ onOpenAuth, onOpenVoice }: NavbarProps) {
  ...
  return (
    <div className="w-full sticky top-0 z-50 select-none">
      ...
      {/* Main Navbar Header */}
      <header className="w-full bg-[#F7EFD9]/95 backdrop-blur-xs border-b border-[#D8CABA]/80 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[76px] sm:h-[80px] flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3.5 group">
            ...
            <div className="flex flex-col">
              <span className="font-headline font-black text-2xl text-[#281E15] tracking-tight leading-tight">
                BeejMantra
              </span>
              <span className="text-[11px] font-sans text-[#5D4A3A] font-semibold tracking-normal -mt-0.5">
              ...
              return (
                <a
                  ...
                  className={`text-[14px] font-semibold transition-all relative py-1 ${
                    isActive
                      ? "text-[#245B35] font-bold"
                      : "text-[#3F2918] hover:text-[#245B35]"
                  }`}
                >
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#245B35] rounded-full shadow-xs" />
                  )}
                </a>
              );
            })}
          ...
          <div className="hidden sm:flex items-center gap-4">
            <div className="text-[#3F2918] hover:text-[#245B35] transition-colors">
              <LanguageSwitcher />
            </div>
            ...
            {user ? (
              ...
              <div className="flex items-center gap-3">
                <Button
                  variant="ghost"
                  onClick={onOpenAuth}
                  className="text-sm font-semibold text-[#3F2918] hover:text-[#245B35] hover:bg-[#FAF5E8] rounded-full px-3.5"
                >
